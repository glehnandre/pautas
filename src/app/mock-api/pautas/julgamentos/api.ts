...
import { julgamentos as julgamentoData, julgamentos, processos as processosData } from './data';
    ...
    registerHandlers(): void {
      ...
      this._fuseMockApiService
        ...
        .reply(({request, urlParams}) => {
          ...
          if (sessaoDeJulgamento) {
            return [200, sessaoDeJulgamento.sessao];
          } else {
            return [404, { description: 'Sessao de julgamento não foi encontrada' }];
          }
        });
        ...
        .reply(({request, urlParams}) => {
          ...
          this._julgamentos
            .map(julg => {
              let numeroano = `${julg.sessao.numero}-${julg.sessao.ano}`;
              if(numeroano == numeroAno){
                julg.sessao.situacao = "REJEITADA";
                sessaoDeJulgamento = julg.sessao;
              }
            });
